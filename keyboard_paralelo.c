...
//				Sw_Pad_White_SetPullup();
			...
			break;
			case 1:
				     if(!Sw_Cancel_GetValue())		{ key.key = 'C'; key.debounce = Ticks()+100;}//printf("Teclado = Cancelar\n\r");}
				else if(!Sw_Acepta_GetValue())		{ key.key = 'A'; key.debounce = Ticks()+100;}//printf("Teclado = Aceptar\n\r");}
				else if(!Sw_Menu_GetValue())		{ key.key = 'M'; key.debounce = Ticks()+100;}//printf("Teclado = Menu\n\r");}
				else if(!Sw_Increment_GetValue())	{ key.key = '+'; key.debounce = Ticks()+100;}//printf("Teclado = Incrementar\n\r");}
				else if(!Sw_Decrement_GetValue())	{ key.key = '-'; key.debounce = Ticks()+100;}//printf("Teclado = Decrementar\n\r");}
				else if(!Sw_PAD_IR_GetValue())		{ key.key = '1'; key.debounce = Ticks()+100;}//printf("Teclado = Pad IR\n\r");}
				else if(!Sw_Pad_White_GetValue())	{ key.key = '2'; key.debounce = Ticks()+100;}//printf("Teclado = Pad Blanco\n\r");}
				key.state = 2;
			break;
			case 2:
